...
/*
 * Mod Name     : PvP Training Dummy
 * Target Engine: MCPE 0.14.3 (ModPE Wrapper - @minecraft/server syntax)
...
// ==========================================
// 2. المتغيرات العامة لإدارة الإحصائيات
// ==========================================

var DUMMY_NAME = "§l§c[PvP Dummy]§r";
var dummyEntityId = null;
var currentCombo = 0;
var hitTimestamps = [];
var lastHitTick = 0;
...
world.afterEvents.entityHurt.subscribe(function(event) {
    ...
    if (victim.nameTag === DUMMY_NAME || (dummyEntityId !== null && victim.id === dummyEntityId)) {
        ...
        if (attacker && attacker.sendMessage) {
            ...
            if (attacker.onScreenDisplay) {
                attacker.onScreenDisplay.setActionBar("§eCombo: " + currentCombo + " §f| §bCPS: " + cps);
            }
        }
    }
});
...
system.runInterval(function() {
    ...
    if (currentCombo > 0 && (system.currentTick - lastHitTick > 40)) {
        ...
        if (players.length > 0) {
            players[0].sendMessage("§c[PvP Dummy] Combo reset. Final Combo Count: " + currentCombo);
        }
        currentCombo = 0;
    }
}, 5);
